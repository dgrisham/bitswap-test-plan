...
func Transfer(runenv *runtime.RunEnv) error {
	...
	runPrefix := runenv.StringParam("run_prefix")
	if err != nil {
		return err
	}
	...
	for sizeIndex, fileSize := range fileSizes {
		...
		for runNum := 1; runNum < runCount+1; runNum++ {
			...
			/// --- Report stats
			err = emitMetrics(runenv, bsnode, runNum, seq, grpseq, latency, bandwidthMB, fileSize, nodetp, tpindex, timeToFetch, runPrefix)
			if err != nil {
				return err
			}
...
func emitMetrics(runenv *runtime.RunEnv, bsnode *utils.Node, runNum int, seq int64, grpseq int64,
	latency time.Duration, bandwidthMB int, fileSize int, nodetp utils.NodeType, tpindex int, timeToFetch time.Duration, runPrefix string) error {
	...
	id := fmt.Sprintf("%s/latencyMS:%d/bandwidthMB:%d/run:%d/seq:%d/groupName:%s/groupSeq:%d/fileSize:%d/nodeType:%s/nodeTypeIndex:%d",
		runPrefix, latencyMS, bandwidthMB, runNum, seq, runenv.TestGroupID, grpseq, fileSize, nodetp, tpindex)
	if nodetp == utils.Leech {
		runenv.R().RecordPoint(utils.MetricTimeToFetch(id), float64(timeToFetch))
	}

	runenv.R().RecordPoint(utils.MetricMsgsRcvd(id), float64(stats.MessagesReceived))
	runenv.R().RecordPoint(utils.MetricDataSent(id), float64(stats.DataSent))
	runenv.R().RecordPoint(utils.MetricDataRcvd(id), float64(stats.DataReceived))
	runenv.R().RecordPoint(utils.MetricDupDataRcvd(id), float64(stats.DupDataReceived))
	runenv.R().RecordPoint(utils.MetricBlksSent(id), float64(stats.BlocksSent))
	runenv.R().RecordPoint(utils.MetricBlksRcvd(id), float64(stats.BlocksReceived))
	runenv.R().RecordPoint(utils.MetricDupBlksRcvd(id), float64(stats.DupBlksReceived))
